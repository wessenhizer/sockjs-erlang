...
json_encode_string(B, State) when is_binary(B) ->
    case json_bin_is_safe(B) of
        true ->
            <<?Q, B/binary, ?Q>>;
        false ->
            json_encode_string_unicode_bin(B)
    end;
...
json_encode_string_unicode_bin(<<C:8, Rest/binary>>, Acc, Buff) ->
    {Acc1, Buff1} = case C of
        $\" -> json_encode_string_unicode_bin_simple(Acc, Buff, $");
        $\\ -> json_encode_string_unicode_bin_simple(Acc, Buff, $\\);
        $\b -> json_encode_string_unicode_bin_simple(Acc, Buff, $b);
        $\f -> json_encode_string_unicode_bin_simple(Acc, Buff, $f);
        $\n -> json_encode_string_unicode_bin_simple(Acc, Buff, $n);
        $\r -> json_encode_string_unicode_bin_simple(Acc, Buff, $r);
        $\t -> json_encode_string_unicode_bin_simple(Acc, Buff, $t);
    ...
    json_encode_string_unicode_bin(Rest, Acc1, Buff1).

json_encode_string_unicode_bin_simple(Acc, Buff, Repl) ->
    Buff0 = json_encode_string_unicode_bin_check(Buff),
    {<<Acc/binary, Buff0/binary, $\\, Repl>>,
     <<"">>}.

json_encode_string_unicode_bin_check(Buff) when byte_size(Buff) < 1 ->
    Buff;
json_encode_string_unicode_bin_check(Buff) ->
    case Buff of
        <<P0:3/integer, D0:5/integer,
          P1:2/integer, D1:6/integer>> when P0==2#110, P1==2#10 ->
            unihex_bin(D0*16#40 + D1);
        <<P0:4/integer, D0:4/integer,
          P1:2/integer, D1:6/integer,
          P1:2/integer, D2:6/integer>> when P0==2#1110, P1==2#10 ->
            unihex_bin(D0*16#1000 + D1*16#40 + D2);
        <<P0:5/integer, D0:3/integer,
          P1:2/integer, D1:6/integer,
          P1:2/integer, D2:6/integer,
          P1:2/integer, D3:6/integer>> when P0==2#11110, P1==2#10 ->
            unihex_bin(D0*16#040000 * D1*16#1000 + D2*16#40 + D3);
